...
class BWFAN_Model_Automation_Complete_Contact extends BWFAN_Model {
	...
	public static function is_contact_with_same_order( $aid, $cid, $order_id, $single_item = 0, $event = 'wc_new_order' ) {
		...
		if ( ! empty( $single_item ) ) {
			$single_item = '%"wc_single_item_id":' . $single_item . '%';
			$data        .= " AND `data` LIKE '$single_item' ";
		}

		$query = "SELECT `ID` FROM `{$wpdb->prefix}bwfan_automation_complete_contact` WHERE `cid` = %d AND `aid` = %d AND `event` = %s  AND $data ORDER BY `ID` DESC LIMIT 0,1";
		$res   = $wpdb->get_var( $wpdb->prepare( $query, $cid, $aid, $event ) ); //phpcs:ignore WordPress.DB.PreparedSQL
