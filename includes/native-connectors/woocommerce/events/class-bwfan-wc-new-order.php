...
final class BWFAN_WC_New_Order extends BWFAN_Event {
	...
	public function get_email_event() {
		if ( ! $this->order instanceof WC_Order ) {
			$this->order = ! empty( $this->order_id ) ? wc_get_order( intval( $this->order_id ) ) : '';
		}

		return $this->order instanceof WC_Order ? $this->order->get_billing_email() : '';
	}
	...
	public function get_user_id_event() {
		...
		$order = ! empty( $this->order_id ) ? wc_get_order( intval( $this->order_id ) ) : '';
	...
	public function capture_v2_data( $automation_data ) {
		...
		if ( 0 === $this->has_run ) {
			$order->delete_meta_data( '_bwfan_v2_' . $this->get_slug() );
			$order->save();
